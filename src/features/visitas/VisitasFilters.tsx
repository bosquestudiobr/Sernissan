...
type VisitasFiltersProps = {
  concessionarias: Option[]
}

export function VisitasFilters({ concessionarias }: VisitasFiltersProps) {
  ...
  function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    for (const key of ['status', 'concessionariaId', 'from', 'to']) {
  ...
  return (
    <form onSubmit={onSubmit} className="mb-4 flex flex-wrap items-end gap-3">
      <div className="space-y-1">
        ...
        <Input name="q" defaultValue={searchParams.get('q') ?? ''} placeholder="Relato" className="h-9 w-full min-w-[180px] bg-[var(--sn-field)]" />
      </div>
      <div className="space-y-1">
