...
export function HabitosFilters({
  concessionarias,
  profiles,
  areas,
  funcoes,
  ...
  function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    for (const key of ['status', 'concessionariaId', 'responsavelId', 'areaId', 'funcaoId', 'from', 'to']) {
  ...
  return (
    <form onSubmit={onSubmit} className="mb-4 flex flex-wrap items-end gap-3">
      ...
      <div className="space-y-1">
        ...
        <select name="areaId" defaultValue={searchParams.get('areaId') ?? ''} className="h-9 min-w-[120px] rounded-md border border-[var(--sn-border)] bg-[var(--sn-field)] px-2 text-sm">
      ...
      <div className="space-y-1">
        ...
        <select name="funcaoId" defaultValue={searchParams.get('funcaoId') ?? ''} className="h-9 min-w-[120px] rounded-md border border-[var(--sn-border)] bg-[var(--sn-field)] px-2 text-sm">
          <option value="">Todas</option>
          {funcoes.map((o) => (
            <option key={o.value} value={o.value}>{o.label}</option>
          ))}
